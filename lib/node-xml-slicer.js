var Slicer = (function () {
    function Slicer(parser, rootPath) {
        var _this = this;
        this.location = [];
                ...
                if (this.errors || this.objectStack.length === 0) {
                    ...
                    this.objectStack.forEach(function (item) {
                        if (_this.result_[item.name]) {
                            ...
                            _this.result_[item.name].push(Slicer.itemResult(item));
                        }
                        else {
                            _this.result_[item.name] = Slicer.itemResult(item);
                        }
                    });
                }
            }
    ...
    Slicer.itemResult = function (item) {
        if (item.result) {
            return item.result;
        }
        else {
